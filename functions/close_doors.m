...
%%Ex. close_doors(obj) or close_doors(obj, DOOR_OFF) 
%%or for a single door close_doors(obj, {'D011'}) 
function CODE_OUT = close_doors(obj, varargin)
    global DOOR_OFF
    CODE_OUT = DOOR_OFF;
    ...
    if(~isempty(varargin))
        ...
        if(isnumeric(DOOR_ID))
            DOOR_ID = {strcat('D0', num2str(DOOR_ID), '1')};
        end
        %%If it is a string,
